...
\sloppypar{Searching for parking spots wastes time while generating
frustration and pollution. To address these parking problems, we present
...
requires no manual event labeling, and users interact with the app only when
looking for parking spots. PocketParker clients detects parking and leaving
events by leveraging existing activity recognition algorithms. Detected
events are used to maintain per-lot availability models allowing the
PocketParker server to respond to client availability queries. By estimating
the number of \textit{hidden drivers}---those not using PocketParker---we can
use a small fraction of drivers to estimate arrival and departure rates and

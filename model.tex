...
for a given destination are full, and drivers are searching unsuccessfully in
all lots. On one hand, if all lots are full then no good ordering of lots
exists and PocketParker's capacity estimates are useless anyway. On the other
hand, we would still like to identify this situation in case users are making
decisions on where to go using parking lot data.
...
a estimate of the number of spots available in the lot scaled by
$\frac{1}{f_m}$, although we do not bound $a_l$ to be below the lot capacity

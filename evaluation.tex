...
of PocketParker that monitored several lots at our university. In two lots,
camera monitoring was used to ground truth the predictions from our
PocketParker deployment. All three components of our evaluation together
confirm that PocketParker is efficiency and accurate.
...
cycle, which is the one that we use. False positive rates are already low,
and this is before we apply our GPS availability filter. False positives
...
generally follow this pattern. Errors for High Churn are high, and Low Churn
errors persist even at high monitored driver fractions. This is expected, as
the Low Churn lot never fills. Fast Fill results improve with increasing
monitored driver fraction. We are investigating, however, the problems with
estimating lot~1 capacity, and persistent errors with the Slow Fill lots.
Improving the accuracy of monitored fraction estimation may require
additional smoothing techniques or incorporating information from other lots.
However, as we demonstrate later, our availability estimator is quite robust
to errors in the monitored fraction up to 100\% of the true value.
